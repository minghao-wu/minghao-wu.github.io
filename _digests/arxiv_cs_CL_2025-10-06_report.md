...
Today's research highlights significant advances in multi-agent collaboration and model alignment, with two key themes emerging. First, **LLM-Hanabi** introduces a novel benchmark demonstrating that first-order Theory-of-Mind—accurately interpreting partners' intents—correlates more strongly with collaborative success than complex higher-order reasoning in imperfect-information games. Second, **BVPO** addresses the critical challenge of aligning Large Reasoning Models by optimizing the bias-variance trade-off in preference optimization, achieving substantial improvements in both alignment and mathematical reasoning capabilities through stable gradient estimation. Meanwhile, analysis of **Multilingual Routing in Mixture-of-Experts** reveals how cross-lingual expert sharing in middle layers directly impacts multilingual generalization, with simple inference-time interventions consistently boosting performance across diverse languages.

## TL;DR

Here's a TL;DR summary of the key papers:

**Core Themes:** Multi-agent collaboration, multilingual reasoning in MoE models, and preference optimization for reasoning models.

**Key Insights:**
- **LLM-Hanabi**: First-order Theory-of-Mind (interpreting others' intent) correlates more strongly with collaborative success than second-order reasoning, providing a clear direction for improving AI collaboration.

- **Multilingual MoE Routing**: MoE models show language-specific routing in early/late layers but cross-lingual alignment in middle layers. Performance in any language strongly correlates with how similarly tokens are routed to English in these middle layers.

- **BVPO for Reasoning Models**: A bias-variance optimized method that combines trace-based and empty-trace gradients significantly improves alignment (+7.8 points on AlpacaEval) while preserving reasoning capabilities, addressing high variance from stochastic trace sampling.

**Common Thread**: All papers focus on improving model capabilities through better understanding of internal mechanisms (routing patterns, gradient variance, reasoning inference) rather than just scaling.
